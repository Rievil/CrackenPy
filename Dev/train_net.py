...
IMAGE_PATH = r"/Users/richarddvorak/Library/CloudStorage/OneDrive-VUT/Kamera/DataSets/GACR/GACR_CrackDataset_Patches/Images"
MASK_PATH = r"/Users/richarddvorak/Library/CloudStorage/OneDrive-VUT/Kamera/DataSets/GACR/GACR_CrackDataset_Patches/Labels"
...
class CrackDataset(Dataset):
    ...
    def __init__(
        self, img_path, mask_path, X, mean, std, transform=None, patch=False
    ):
    ...
    def __getitem__(self, idx):
        ...
        mask = cv2.imread(
            self.mask_path + "\\" + self.X[idx] + ".png", cv2.IMREAD_GRAYSCALE
        )
...
def mIoU(pred_mask, mask, smooth=1e-10, n_classes=4):
    with torch.no_grad():
        ...
        for clas in range(0, n_classes):  # loop per pixel class
            ...
            if (
                true_label.long().sum().item() == 0
            ):  # no exist label in this loop
                iou_per_class.append(np.nan)
            else:
                intersect = (
                    torch.logical_and(true_class, true_label)
                    .sum()
                    .float()
                    .item()
                )
                union = (
                    torch.logical_or(true_class, true_label)
                    .sum()
                    .float()
                    .item()
                )
...
history = fit(
    epoch, model, train_loader, val_loader, criterion, optimizer, sched
)
...
def predict_image(
    model, image, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
):
    model.eval()
...
def plot_loss(history):
    plt.plot(
        history["val_loss"], "-o", label="Loss validation", color="tab:blue"
    )
    plt.plot(
        history["train_loss"], ":o", label="Loss training", color="tab:blue"
    )

    # def plot_score(history):
    plt.plot(
        history["train_miou"],
        label="train $\overline{IoU}$",
        marker="*",
        color="tab:orange",
    )
    plt.plot(
...
fig.savefig(
    "Plots\Training_310124.pdf", dpi=300, bbox_inches="tight", pad_inches=0
)
# plot_score(history)
# plot_acc(history)
...
class DroneTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        mask = cv2.imread(
            self.mask_path + r"\\" + self.X[idx] + ".png", cv2.IMREAD_GRAYSCALE
        )
...
def COM2(model, test_set, n_classes=23):
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            matrix[predict_pixel][target_pixel] = (
