...
fig.savefig(
    "Plots\DeepLearning_example_mask.png",
    dpi=300,
    bbox_inches="tight",
    pad_inches=0,
)
...
fig.savefig(
    "Plots\Class_Test_Acuracy.pdf", dpi=300, bbox_inches="tight", pad_inches=0
)
...
for class_num, ax in zip([0, 1, 2, 3], axes.flat):
    ...
    mIOU = mIOU + IOU
    im = ax.imshow(
        mask_r, alpha=0.8, cmap=my_cmap, interpolation="antialiased"
    )
    ...
    ax.set_title(
        r"Class: '{:s}' IOU:{:0.2f}".format(classlist[class_num], IOU)
    )

plt.suptitle(
    "Mean intersection over union {:s}: {:0.4f}".format(
        r"$\overline{IOU}$", mIOU / 4
    )
)
...
cbar.ax.set_yticklabels(
    ["Empty", "True Positive", "False Positive", "False Negative"]
)
fig.savefig(
    "Plots\IOU_Example.pdf", dpi=300, bbox_inches="tight", pad_inches=0
)
...
fig.savefig(
    "Plots\Pixel_mm_Ratio.pdf", dpi=300, bbox_inches="tight", pad_inches=0
)
# ax.axis((0, 600, 600, 0))
plt.show()
# %%
...
image = bw_mask
coords = corner_peaks(
    corner_harris(image), min_distance=100, threshold_rel=0.02
)
...
tit_str = "Crack ratio: {:.4f}, Length: {:.0f}, Thickness: {:.2f}".format(
    meas["CrackRatio"], meas["TotalCrackLength"], meas["MeanCrackThickness"]
)
plt.title(tit_str)
print(tit_str)
# %%
